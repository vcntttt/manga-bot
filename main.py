...
def loadCSV():
    mangas = []
    with open('/home/vrivera/dev/tmo-bot/mangas.csv', 'r', encoding='utf-8') as file:
...
def updateCSV(mangas):
    with open('/home/vrivera/dev/tmo-bot/mangas.csv', 'w', newline='', encoding='utf-8') as file:
...
async def scheduledCheck():
    ...
    if channel:
        #await channel.send('Comprobando capitulos...')
        mangas = loadCSV()
        for manga in mangas:
